...
class RobertaSpamClassifier(AbstractSpamClassifier):
    def __init__(self, de_to_en_translator: Union[AbstractTextTranslator, None] = None):
        super().__init__(de_to_en_translator)

        self.classifer = pipeline('text-classification', model='mshenoda/roberta-spam')

    @staticmethod
    def get_name() -> str:
        return 'RobertaSpamClassifier (mshenoda/roberta-spam)'
